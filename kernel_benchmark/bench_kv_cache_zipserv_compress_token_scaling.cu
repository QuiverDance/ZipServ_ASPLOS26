/***************************************************************************
 * KV Cache ZipServ Token Compression Scaling Benchmark
 *
 * Measures compression latency for the first N tokens of each KV-cache file
 * across three CPU methods (Single / MT / MT-AVX / bitcomp) and two GPU
 * methods (ZipServ-GPU / bitcomp-GPU).
 *
 * Default behavior:
 *   Compress the full N-token prefix as a single workload.
 *
 * Optional chunking:
...
static const char* kGPUMethodNames[] = {"ZipServ-GPU", "bitcomp-GPU"};
...
struct ProgramOptions {
    ...
    int max_files = -1;
    int recursive = 0;
    std::string out_csv = "kv_cache_zipserv_token_scaling_results.csv";
...
void PrintUsage() {
    std::cout
        << "Usage: ./bench_kv_cache_zipserv_compress_token_scaling [options]\n"
        << "Options:\n"
        ...
        << "  --out_csv <path>        Output CSV path (default: kv_cache_zipserv_token_scaling_results.csv)\n"
...
int main(int argc, char** argv) {
    ...
    std::cout << "===== KV Cache ZipServ Token Compression Scaling Benchmark =====\n";
    ...
    std::cout << "[token mode] each iteration times all eligible files together, then normalizes by processed_files.\n\n";
    ...
    if (opts.mode == "cpu") {
        ...
        bool cpu_processed_mismatch_seen = false;
        ...
        for (size_t sweep_idx = 0; sweep_idx < sweep_values.size(); ++sweep_idx) {
            ...
            if (!BuildWorkItems(token_count, preloaded, num_available,
                                opts.require_full_token_count != 0,
                                &work_items, &eligible_files, &work_error)) {
                std::cerr << "[warning] skipping token_count="
                          << token_count << ": " << work_error << "\n";
                continue;
            }
            ++runnable_points;

            int effective_chunk_len = 0;
            int max_chunks_per_file = 1;
            for (size_t wi = 0; wi < work_items.size(); ++wi) {
                const int64_t resolved_chunk_len =
                    ResolveChunkLenForWorkItem(work_items[wi].seq_count, opts.chunk_len);
                effective_chunk_len = std::max(
                    effective_chunk_len,
                    static_cast<int>(resolved_chunk_len));
                const int64_t num_chunks =
                    (work_items[wi].seq_count + resolved_chunk_len - 1) / resolved_chunk_len;
                max_chunks_per_file = std::max(
                    max_chunks_per_file, static_cast<int>(num_chunks));
            }

            double method_latencies[METHOD_COUNT] = {0.0};
            double method_ratios[METHOD_COUNT] = {0.0};
            int method_processed_files[METHOD_COUNT] = {0};
            ...
            for (int m = 0; m < METHOD_COUNT; ++m) {
                ...
                if (method == METHOD_SINGLE) {
                    ...
                    omp_set_num_threads(std::min(saved_max_threads, max_chunks_per_file));
                }
                ...
                const size_t max_bf16_numel = static_cast<size_t>(benchmark_max_padded_rows)
                ...
                bool alloc_ok = true;
                for (int t = 0; t < num_threads; ++t) {
                    if (!AllocReusableBuffers(benchmark_max_padded_rows, benchmark_max_padded_cols,
                ...
                if (method == METHOD_BITCOMP) {
                    const size_t max_input_bytes =
                        static_cast<size_t>(benchmark_max_padded_rows) * benchmark_max_padded_cols * 2;
                ...
                for (int w = 0; w < opts.warmup; ++w) {
                    ProcessAllFiles(work_items, preloaded, opts.chunk_len, method,
                        thread_bufs, thread_bf16_bufs,
                        nullptr, (method == METHOD_BITCOMP) ? &bitcomp_ctxs : nullptr, nullptr);
                }
                ...
                const double avg_ms = total_ms / static_cast<double>(valid_iters);
                method_latencies[m] = avg_ms;

                // Collect compression ratio (one extra untimed pass)
                CompressionStats comp_stats;
                int processed_files_ratio = 0;
                ProcessAllFiles(work_items, preloaded, opts.chunk_len, method,
                    thread_bufs, thread_bf16_bufs, &comp_stats,
                    (method == METHOD_BITCOMP) ? &bitcomp_ctxs : nullptr,
                    &processed_files_ratio);
                const double ratio = (processed_files_ratio > 0) ? comp_stats.Ratio() : 0.0;
                method_ratios[m] = ratio;
                method_processed_files[m] = processed_files_ratio;

                GenericResult gr;
                gr.token_count = token_count;
                gr.eligible_files = eligible_files;
                gr.processed_files = processed_files_ratio;
                gr.requested_chunk_len = opts.chunk_len;
                gr.effective_chunk_len = effective_chunk_len;
                gr.method_name = kMethodNames[m];
                gr.latency_ms = avg_ms;
                gr.core_latency_ms = 0.0;
                gr.comp_ratio = ratio;
            ...
            for (int m = 0; m < METHOD_COUNT; ++m) {
                if (method_latencies[m] < 0.0) continue;
                if (processed_ref < 0) {
            ...
            std::string processed_display = (processed_ref < 0) ? "-" : std::to_string(processed_ref);
            if (processed_mismatch) {
                processed_display += "*";
                cpu_processed_mismatch_seen = true;
            }
            std::cout << std::left << std::setw(12) << token_count
                      << std::setw(14) << eligible_files
                      << std::setw(16) << processed_display
                      << std::setw(12) << effective_chunk_len;
            for (int m = 0; m < METHOD_COUNT; ++m) {
                std::cout << std::setw(14)
                          << ((method_latencies[m] < 0.0) ? "FAILED" : FormatDouble(method_latencies[m]));
            }
            double zipserv_ratio = -1.0;
            for (int m = METHOD_SINGLE; m <= METHOD_MT_AVX; ++m) {
                if (method_latencies[m] >= 0.0) {
                    zipserv_ratio = method_ratios[m];
                    break;
                }
            }
            std::cout << std::setw(14)
                      << ((zipserv_ratio < 0.0) ? "-" : (FormatDouble(zipserv_ratio, 2) + "x"))
                      << std::setw(14)
                      << ((method_latencies[METHOD_BITCOMP] < 0.0) ? "-" : (FormatDouble(method_ratios[METHOD_BITCOMP], 2) + "x"));
            std::cout << "\n";
            std::cout.flush();
        }

        std::cout << std::string(138, '-') << "\n";
        if (cpu_processed_mismatch_seen) {
            std::cout << "note: ProcessedFiles '*' means method-specific processed file counts differ.\n";
        }

    } else {
        // ========== GPU MODE ==========
        cudaStream_t bitcomp_stream;
        cudaStreamCreate(&bitcomp_stream);

        std::vector<GpuPipelineSlot> zipserv_slots;
        if (!AllocGpuPipelineSlots(benchmark_max_padded_rows, benchmark_max_padded_cols, &zipserv_slots)) {
            std::cerr << "AllocGpuPipelineSlots failed.\n";
            cudaStreamDestroy(bitcomp_stream);
            return 1;
        }

        // Allocate bitcomp GPU context
        const size_t max_input_bytes =
            static_cast<size_t>(benchmark_max_padded_rows) * benchmark_max_padded_cols * 2;
        BitcompGPUContext bc_gpu;
        std::string bc_error;
        if (!AllocBitcompGPUContext(max_input_bytes, bitcomp_stream, &bc_gpu, &bc_error)) {
            std::cerr << "AllocBitcompGPUContext failed: " << bc_error << "\n";
            FreeGpuPipelineSlots(&zipserv_slots);
            cudaStreamDestroy(bitcomp_stream);
            return 1;
        }
        ...
        std::cout << std::left
                  << std::setw(12) << "TokenCount"
                  << std::setw(12) << "ChunkLen"
                  << std::setw(24) << "ZipServ-GPU(ms)"
                  << std::setw(24) << "ZipServ-GPU API(ms)"
                  << std::setw(18) << "ZipServ-GPU Comp"
                  << std::setw(24) << "bitcomp Comp(ms)"
                  << std::setw(18) << "bitcomp-GPU Comp"
                  << "\n";
        ...
        for (size_t sweep_idx = 0; sweep_idx < sweep_values.size(); ++sweep_idx) {
            ...
            for (size_t wi = 0; wi < work_items.size(); ++wi) {
                effective_chunk_len = std::max(
                    effective_chunk_len,
                    static_cast<int>(ResolveChunkLenForWorkItem(
                        work_items[wi].seq_count, opts.chunk_len)));
            }

            double gpu_latencies[GPU_METHOD_COUNT] = {0.0};
            double gpu_core_latencies[GPU_METHOD_COUNT] = {0.0};
            double gpu_ratios[GPU_METHOD_COUNT] = {0.0};

            for (int m = 0; m < GPU_METHOD_COUNT; ++m) {
                const GPUCompressMethod method = static_cast<GPUCompressMethod>(m);

                // Warmup
                for (int w = 0; w < opts.warmup; ++w) {
                    ProcessAllFiles_GPU(work_items, preloaded, opts.chunk_len, method,
                        &zipserv_slots, &bc_gpu, bitcomp_stream,
                ...
                int valid_iters = 0;
                for (int iter = 0; iter < opts.iters; ++iter) {
                    double iter_core_ms = 0.0;
                    int processed_files_iter = 0;
                    const auto wall_begin = std::chrono::high_resolution_clock::now();
                    ProcessAllFiles_GPU(work_items, preloaded, opts.chunk_len, method,
                        &zipserv_slots, &bc_gpu, bitcomp_stream,
                        nullptr, &iter_core_ms, &processed_files_iter);
                    const auto wall_end = std::chrono::high_resolution_clock::now();
                    if (processed_files_iter <= 0) continue;
                    const double elapsed_ms =
                        std::chrono::duration<double, std::milli>(wall_end - wall_begin).count();
                    total_ms += elapsed_ms / static_cast<double>(processed_files_iter);
                    total_core_ms += iter_core_ms / static_cast<double>(processed_files_iter);
                    valid_iters += 1;
                }

                if (valid_iters <= 0) {
                    gpu_latencies[m] = -1.0;
                    gpu_core_latencies[m] = 0.0;
                    gpu_ratios[m] = 0.0;
                    continue;
                }

                const double avg_ms = total_ms / static_cast<double>(valid_iters);
                const double avg_core_ms = total_core_ms / static_cast<double>(valid_iters);
                gpu_latencies[m] = avg_ms;
                gpu_core_latencies[m] = avg_core_ms;

                // Collect compression ratio (one extra untimed pass)
                CompressionStats comp_stats;
                int processed_files_ratio = 0;
                ProcessAllFiles_GPU(work_items, preloaded, opts.chunk_len, method,
                    &zipserv_slots, &bc_gpu, bitcomp_stream, &comp_stats, nullptr,
                    &processed_files_ratio);
                const double ratio = (processed_files_ratio > 0) ? comp_stats.Ratio() : 0.0;
                gpu_ratios[m] = ratio;
                ...
                gr.requested_chunk_len = opts.chunk_len;
                gr.effective_chunk_len = effective_chunk_len;
                gr.method_name = kGPUMethodNames[m];
                gr.latency_ms = avg_ms;
                gr.core_latency_ms = avg_core_ms;
                gr.comp_ratio = ratio;
                results.push_back(gr);
            }

            // Print row
            const int z = static_cast<int>(GPU_METHOD_ZIPSERV);
            const int b = static_cast<int>(GPU_METHOD_BITCOMP);
            std::cout << std::left << std::setw(12) << token_count
                      << std::setw(12) << effective_chunk_len;
            if (gpu_latencies[z] < 0.0) {
                std::cout << std::setw(24) << "FAILED"
                          << std::setw(24) << "-"
                          << std::setw(18) << "-";
            } else {
                std::cout << std::setw(24) << FormatDouble(gpu_latencies[z])
                          << std::setw(24) << FormatDouble(gpu_core_latencies[z])
                          << std::setw(18) << (FormatDouble(gpu_ratios[z], 2) + "x");
            }
            if (gpu_latencies[b] < 0.0) {
                std::cout << std::setw(24) << "-"
                          << std::setw(18) << "-";
            } else {
                std::cout << std::setw(24) << FormatDouble(gpu_core_latencies[b])
                          << std::setw(18) << (FormatDouble(gpu_ratios[b], 2) + "x");
            }
            std::cout << "\n";
            std::cout.flush();
        }
